...
| 메트릭 | 계산법 | NaN 발생 조건 |
|--------|--------|--------------|
| Context Recall | `TP / (TP + FN)` | reference_contexts 문장이 없거나 평가 LLM 호출 실패 |
| Context Precision | retrieved 청크 각각의 관련도 평균 | retrieved_contexts가 비었거나 관련 청크가 하나도 없을 때 |
| Faithfulness | `근거 있는 claim / 전체 claim` | response에서 claim 추출 실패 (예: "정보를 찾을 수 없습니다") |
| Answer Relevancy | 역질문과 원래 질문의 임베딩 유사도 평균 | response가 너무 짧거나 역질문 생성 실패 |
| Answer Correctness | `2*TP / (2*TP + FP + FN)` | claim 추출 자체 실패 (API 에러, JSON 파싱 실패) |

> TP = 맞게 말한 것, FP = 틀리게 말한 것, FN = 말했어야 하는데 안 한 것

**주요 관찰**

- **NaN 0건**: MD 클리닝 후 재구성한 FAISS 인덱스 사용으로 NaN이 완전히 사라짐
- **모든 메트릭에서 Advanced > Basic**: 이전에 NaN으로 손실됐던 점수들이 정상 반영된 결과
- **q06**: Advanced가 Basic 대비 크게 개선 (0.040 → 0.861) — Hybrid+Reranking이 올바른 청크를 검색한 케이스
- **q03, q20**: Basic이 Advanced보다 높음 (q03: 0.806 → 0.623, q20: 0.824 → 0.655) — Re-ranking 역효과
- **근본 원인**: Context Recall 0.0 문항(q01·q05·q07·q08·q09·q13·q15·q17·q18·q19)은 child=300 청크 크기에서 HTML 표 구조가 분절되어 정보 손실 발생

### 5-3. 수동 채점 vs Ragas 비교

4주차 수동 판정(정답/오답)과 5주차 Ragas Answer Correctness 점수를 비교했다. 일치 기준: 정답이면 0.5 이상, 오답이면 0.5 미만.

**Basic RAG**

| ID | 수동 판정 | Ragas Ans Correctness | 일치 여부 | 불일치 원인 |
|----|---------|----------------------|---------|-----------|
| q01 | 오답 | 0.209 | 일치 | |
| q02 | 정답 | 0.649 | 일치 | |
| q03 | 정답 | 0.806 | 일치 | |
...
| q09 | 오답 | 0.205 | 일치 | |
| q10 | 정답 | 0.632 | 일치 | |
| q11 | 정답 | 0.424 | 불일치 | "1,500~2,000원" 답변 → ground_truth "1,500원"과 표현 차이로 저평가 |
| q12 | 정답 | 0.663 | 일치 | |
| q13 | 오답 | 0.039 | 일치 | |
| q14 | 정답 | 0.669 | 일치 | |
| q15 | 오답 | 0.043 | 일치 | |
...
| q15 | 오답 | 0.192 | 일치 | |
| q16 | 오답 | 0.209 | 일치 | |
| q17 | 정답 | 0.213 | 불일치 | 정답이지만 ground_truth 문장과 표현이 달라 Ragas 저평가 |
| q18 | 정답 | 0.212 | 불일치 | 정답이지만 ground_truth 문장과 표현이 달라 Ragas 저평가 |
| q19 | 정답 | 0.708 | 일치 | |
...
| 항목 | Basic | Advanced |
|------|-------|---------|
| 수동 정답률 | 40% (8/20) | 45% (9/20) |
| Ragas Ans Correctness 평균 | 0.368 | 0.457 |
| 수동↔Ragas 일치율 | 90% (18/20) | 70% (14/20) |
| 불일치 주요 원인 | 수치 형식 유사성(q07), 표현 차이(q11) | 수치 형식 유사성(q03·q06·q07·q20), 표현 차이(q17·q18) |

- 수동 채점과 Ragas는 대체로 일치하나, 틀린 답변에도 형식 유사성으로 Ragas가 부분 점수를 부여하는 케이스 발생
- Ragas가 수동 채점보다 항상 엄격하지는 않음: 수동 오답이 Ragas 0.6 이상으로 평가되는 케이스 다수 존재
- Advanced 일치율(70%)이 Basic(90%)보다 낮은 이유: Re-ranking이 답변 표현의 다양성을 높이고, Ragas Answer Correctness는 claim 단위 F1이라 ground_truth와 표현이 달라지면 정답이어도 낮게 측정될 수 있음

---

## 6. Basic vs Advanced 비교 분석
...
- Context Recall은 년도 혼동을 구분하지 못함 — cross-year 4문항에서 Basic/Advanced 모두 동일한 값. 년도 혼동이 드러나는 건 Answer Correctness
- Re-ranking이 년도 혼동에 일관되지 않게 작동함: q19에서는 2026년 청크를 올바르게 선택, q20에서는 2025년 청크를 상위로 올려 역효과
- Ragas 기본 메트릭은 년도 혼동을 직접 포착하지 못함. Year Accuracy 커스텀 메트릭이 필요한 이유

### 6-3. 인사이트

**"Advanced가 낫다" 결론의 재검토**

MD 클리닝 후 모든 메트릭에서 Advanced > Basic으로 나타났다. 이전 결과에서 일부 메트릭이 악화됐던 것은 NaN으로 손실된 점수가 평균을 왜곡했던 영향이다. 다만 Re-ranking이 모든 문항에서 일관되게 좋은 건 아님: q03·q20에서 정답 청크를 탈락시키는 역효과가 확인된 반면, q19에서는 올바른 년도 청크를 선택해 크게 개선됐다(0.050 → 0.708).
...
1. **청크 크기 확대 (c1000, p3000)**: Context Recall 0.0 문항이 절반(10/20)에 달하며, HTML 표 구조 보존이 핵심. 4주차에서 이미 70% 달성 확인.
2. **Faithfulness 개선**: 컨텍스트 외 정보 사용 금지 프롬프트 강화.
3. **Re-ranker 년도 인식 개선**: source_year 메타데이터 기반 년도 pre-filtering으로 년도 혼동 방지.

---

## 7. 실패 케이스 Deep Dive

### Case A: q03 — Re-ranking 역효과로 Advanced가 Basic보다 악화
...
- Advanced Re-ranking 과정에서 자연분만 면제 청크가 탈락하고 비관련 청크가 최종 retrieved_contexts로 전달된 것으로 추정
- Context Recall 1.0 · Precision 1.0임에도 Faithfulness 0.400 · Correctness 0.623이라는 내부 모순 발생 → 평가 LLM(Claude Sonnet)이 주제 유사성만으로 관련 있다고 판단한 오평가 가능성
- **조치**: 청크 크기 확대(c1000, p3000)로 분만 면제 규정 전체를 하나의 청크에 포함

### Case B: q20 — 년도 혼동 (청크는 검색됐지만 답변 년도 틀림)
...
- Advanced Re-ranking 후 2025년 5% 청크가 상위로 올라오고 2026년 2% 청크가 낮게 스코어링된 것으로 추정 → LLM이 2025년 기준으로 답변 생성
- Context Precision이 Advanced에서 더 높음(1.0 vs 0.333)에도 Correctness는 반대 — Precision이 높아도 정답 년도 청크가 아닐 수 있음을 보여주는 케이스
- Ragas 기본 메트릭은 "2026년 정보를 검색했는가"를 직접 측정하지 못함
- **조치**: source_year 메타데이터 기반 년도 pre-filtering 적용

### Case C: q09 — Faithfulness와 Answer Correctness 충돌
...
- Advanced Faithfulness 0.750(환각 낮음)이지만 Correctness 0.192(정답과 크게 다름) — 충돌하는 이유는 검색된 청크 자체가 질문과 무관한 경우로, 환각은 없지만 잘못된 컨텍스트에 충실한 상태
- Context Recall 0.0이 실패의 진짜 원인 — 청크 분절로 CT/MRI/PET 표가 분리됨
- **조치**: 청크 크기 확대로 CT/MRI/PET 표 전체를 하나의 청크에 포함

### 공통 교훈

- **Context Recall이 핵심 지표**: Recall 0.0이면 나머지 메트릭은 무의미. 10/20 문항이 해당하며 청크 분절이 근본 원인
- **Ragas가 놓치는 실패 유형**: 년도 혼동 미감지, 잘못된 컨텍스트에 충실한 답변을 Faithfulness 높음으로 평가
- **Re-ranking 역효과는 케이스별로 다름**: q03·q20에서 역효과, q19에서는 크게 개선(0.050 → 0.708). Re-ranker 성능은 청크 품질에 크게 의존
