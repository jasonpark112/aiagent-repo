...
- `copayment_data.md`: 의료급여 본인부담률 참조 표 (system prompt에 포함)
- `dataset.jsonl`: 질문 30건 + 정답 + 난이도
...
`copayment_data.md`의 내용을 system prompt에 넣고, 질문만 user message로 보냅니다.
...
30건 전체를 돌리고 `expected_answer`와 비교하여 **정답률**을 기록합니다.
