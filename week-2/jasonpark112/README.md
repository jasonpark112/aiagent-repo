...
1. Zero-shot
    - hard 에서 정답을 아예 틀림
    - 정답은 맞으나 기대한 출력 값과 일치하지 않아서 틀렸다고 표기가 됨
2. Few-shot
    - hard에서 높은 정답률을 보임
    - 정답은 맞으나 기대한 출력 값과 일치하지 않아서 틀렸다고 표기가 됨
3. CoT
...
## 결론과 인사이트
- 현재 LLM 모델 자체가 기본적인 추론 성능이 좋기 때문에 Zero-shot 에서 예시를 추가한 Few-shot으로도 높은 정답률을 낼 수 있다.
- 현재 LLM 모델이 처리하기엔 복잡한 문제가 있다면 CoT 추론 단계를 추가해주고 더 성능을 높이기 위해서는 추론 단계 결과를 다수결로 내는 Self-consistency 방식을 활용하여 정답률 높이는 것이 좋다.