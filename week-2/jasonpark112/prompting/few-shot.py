...
FEW_SHOT_EXAMPLES = [
    {
        "id": "q01",
        "question": "65세 이상 1종 수급권자가 틀니를 하면 본인부담률은 몇 %인가요?",
        "answer": "5%",
        "reason": "65세 이상 1종 수급권자 틀니 규칙 적용"
    },
    {
        "id": "q02",
        "question": "65세 이상 2종 수급권자가 임플란트를 하면 본인부담률은 몇 %인가요?",
        "answer": "20%",
        "reason": "65세 이상 2종 수급권자 임플란트 규칙 적용"
    },
    {
        "id": "q03",
        "question": "2종 수급권자인 생후 8개월 아기가 만성질환자이고 제2차의료급여기관에서 외래 진료를 받으면 본인부담률은?",
        "answer": "무료",
        "reason": "1세 미만 만성질환자 외래 제2차의료급여기관 규칙 적용"
    },
]
...
# 8. 단건 추론
# ---------------------------
def solve_few_shot(question: str) -> CopaymentResult:
...
# ---------------------------
# 9. 평가 데이터 구성
#    few-shot 예시로 사용한 문항 제외
# ---------------------------
eval_dataset = [item for item in dataset if item["id"] not in FEW_SHOT_IDS]
...
print(f"Step 2 Few-shot Accuracy: {correct}/{len(eval_dataset)} = {accuracy:.2%}")

Path("../outputs").mkdir(exist_ok=True)
with open("../outputs/step2_few_shot_results.json", "w", encoding="utf-8") as f:
